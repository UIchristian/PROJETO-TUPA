...
const LAYERS_CONFIG = [
  ...
  {
    id: "USO_RESTRITO_ENCOSTA",
    label: "Uso Restrito – Encosta",
    color: "#f97316",
    icon: MountainSnow,
  },
] as const;
...
function ScoreRing({ score }: { score: number }) {
  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      ...
      <span className="absolute font-bold text-2xl" style={{ color: c }}>
        {score}
      </span>
    </div>
  );
...
export default function MapaScreen() {
  ...
  const overlays = Object.fromEntries([...activeLayers].map((t) => [t, layerCache[t] ?? null]));
      ...
      <div className="flex items-center gap-3 px-6 py-3 border-b border-border bg-muted/30 flex-shrink-0 overflow-x-auto">
        ...
        {stats ? (
          <>
            ...
            <StatCard label="Em Conformidade" value={`${conformes}%`} sub={`score ≥ 90`} />
            <StatCard label="Score Médio" value={stats.mediaScore.toFixed(1)} sub="de 100 pts" />
            <StatCard
              label="APP Mapeada"
          ...
          {loadingMapa && (
            <div className="absolute inset-0 z-[1000] flex flex-col items-center justify-center bg-background/80">
              ...
              <p className="text-sm text-muted-foreground font-mono">
                Carregando {MUNICIPIO.replace(/_/g, " ")}...
              </p>
            </div>
          )}
          {!import.meta.env.SSR && (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-2 flex items-center gap-1.5">
                      ...
                      <span
                        ...
                        style={{ color: scoreColor(resumo.scoreConformidade) }}
                      >
                        {resumo.scoreConformidade >= 90
                          ? "Em Conformidade"
                          : resumo.scoreConformidade >= 70
                    ...
                    <div className="space-y-2">
                      {resumo.divergencias.map((d, i) => (
                        <div key={i} className="flex items-start gap-2.5 text-xs">
                          <span
                            className={`mt-0.5 px-1.5 py-0.5 text-[9px] font-mono font-black uppercase rounded-none flex-shrink-0 ${
                              d.severidade === "alta"
