  ...
  return (
    <MobileFrame withNav>
      ...
      <section className="bg-alert text-alert-foreground px-5 pt-6 pb-8">
        <div className="flex items-center gap-2 text-[12.5px] tracking-[0.15em] font-bold text-white">
          <Siren size={14} /> {t("gatilho.main_title").toUpperCase()}
        </div>
        <h1 className="text-[22px] font-bold mt-2 leading-tight">
          {t("gatilho.h1_title", { name: farmer.name.split(" ")[0] })}
        </h1>
        <p className="text-[14px] text-white/95 mt-2 leading-relaxed">{t("gatilho.payout_desc")}</p>
      </section>
      ...
      <section className="bg-background rounded-t-3xl -mt-5 px-5 pt-6 pb-6 flex flex-col gap-5 relative z-10">
        ...
        <div className="rounded-2xl bg-primary text-primary-foreground p-5 shadow-soft">
          <p className="text-[13.5px] text-white font-medium">{t("gatilho.value")}</p>
          <div className="flex items-baseline gap-1 mt-1">
            <span className="text-[15px] money">R$</span>
            <CountUp to={4000} className="text-[40px] money leading-none" />
          </div>
          <p className="text-[14.5px] text-white/95 mt-1.5 flex items-center gap-1.5 font-semibold">
        ...
        {/* Status tracker */}
        <div>
          <h2 className="text-[16px] font-bold mb-3">{t("gatilho.card_status")}</h2>
                ...
                <li key={s.key} className="flex items-start gap-3">
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 transition-colors ${
                        ...
                        : active
                          ? "bg-amber-warn text-white animate-pulse"
                  ...
                  <div className="flex-1 pt-0.5">
                    ...
                    >
                      {s.label}
                      {active && (
                        <span className="ml-2 text-[12.5px] font-bold text-amber-warn">
                          {t("gatilho.in_progress")}
                        </span>
                      )}
                      {done && (
                        <span className="ml-2 text-[12.5px] font-bold text-primary">
                          {t("gatilho.completed")}
                        </span>
                      )}
                    </p>
                    <p className="text-[13.5px] text-foreground/80 mt-0.5">{s.hint}</p>
                  </div>
                </li>
              );
            })}
        ...
        <Link
          to="/lavoura"
          className="h-14 rounded-2xl bg-navy text-navy-foreground font-semibold text-[16px] flex items-center justify-center gap-2 active:scale-[0.99] shadow-soft"
        >
...
function Metric({ value, label, hint }: { value: string; label: string; hint?: string }) {
  return (
    <div>
      <div className="text-[20px] money text-navy leading-none">{value}</div>
      <div className="text-[12px] text-foreground/80 font-bold mt-1.5 leading-tight">{label}</div>
      {hint && <div className="text-[10.5px] text-foreground/70 font-semibold mt-0.5">{hint}</div>}
    </div>
  );
}
